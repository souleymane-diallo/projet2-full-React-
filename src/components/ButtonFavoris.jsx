...
function ButtonFavoris(props) {
  const fav = props.recordDataCard;
  ...
  const handleAddFavoriteEvent = (data) => {
    ...
    if (localStorage.getItem('Eventfavorites')) {
      const saveEvent = JSON.parse(localStorage.getItem('Eventfavorites'));
      saveEvent.push(data);
      localStorage.setItem('Eventfavorites', JSON.stringify(saveEvent));
      //   console.log('Ajouter au localStorage', record, record.record.id);
    } else {
      localStorage.setItem('Eventfavorites', JSON.stringify([data]));
      // console.log('non existant ajouter', data);
    }
    setSave(true);
  };
  /**
  ...
  return (
    <div>
      {save ? (
        <span
          className="heart"
          onClick={() => handleDeleteFavoriteEvent(fav)}
          style={{ color: 'red' }}
        >
          <svg
        ...
        <span
          className="heart"
          onClick={() => handleAddFavoriteEvent(fav)}
          style={{ color: 'red' }}
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
